...
type Escrow = { id: string; deposited_amount: number; released_amount: number; refunded_amount: number; status: string; deposit_tx_hash: string | null; escrow_wallet: string | null; token: string | null };
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-lg font-semibold text-slate-900">{esc.deposited_amount} {esc.token ?? "USDC"}</p>
        </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        {ms.map((m) => (
          <Step key={m.idx} done={m.status === "released"} active={m.status === "completed"} label={`${m.name} · ${m.percentage}% (${m.amount} ${esc.token ?? "USDC"})`} sub={m.status} />
        ))}
        <Step done={releasedAll} label="Released to GlobalCare" />
      </div>

      <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
        <span>Escrow {short(esc.escrow_wallet)} · Sepolia</span>
        {esc.deposit_tx_hash && <a href={`https://sepolia.etherscan.io/tx/${esc.deposit_tx_hash}`} target="_blank" rel="noreferrer" className="ml-2 max-w-[45%] truncate font-mono text-blue-500 underline decoration-dotted hover:text-blue-600">{esc.deposit_tx_hash}</a>}
      </div>
    </div>
  );
}
