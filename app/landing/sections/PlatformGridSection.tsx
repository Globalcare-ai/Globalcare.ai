...
/**
 * Adapted from Zepa UI — girl-grid (https://zepa.design/components/girl-grid)
 * Re-themed for GlobalCare.ai: a care-concierge bento.
 *
 * Five tiles on soft grey, laid out wide and shallow so the board reads as one
 * band rather than a wall. The portrait sits under a frosted top band and
 * carries a conversation that arrives bubble by bubble, then starts over. The
 * language card is a real set of switches. The omni-channel tile floats its
 * channels along an arc that keeps tracing itself, and its microphone records
 * when pressed. The journeys counter sits under a folder that fans its patient
 * files out on hover.
 */
...
/** the journeys the counter counts, as a folder you can fan open */
const FILES = [
  { id: "marcus", image: "/x2.webp", title: "Marcus · Istanbul" },
  { id: "linda", image: "/x3.avif", title: "Linda · Mumbai" },
  { id: "daniel", image: "/x4.jpg", title: "Daniel · Istanbul" },
  { id: "ahmed", image: "/x5.jpg", title: "Ahmed · Dubai" },
];

/** the plain row under the highlighted one */
const TASKS = [
  { tone: "cool", text: "Book flights, hotel and airport transfers" },
] as const;

type Msg = { from: "them" | "us"; text: string };

const THREAD: Msg[] = [
  { from: "us", text: "I need a hair transplant. What would it cost outside the US?" },
  { from: "them", text: "Istanbul — about $2,400 all-in. Three accredited clinics match your case." },
  { from: "us", text: "Book it, and hold my payment in escrow until I land." },
];
...
function Flag({ code }: { code: string }) {
  ...
  if (code === "TR")
    return (
      <svg {...box} aria-hidden>
        <rect width="51" height="20" fill="#e30a17" />
        <circle cx="19" cy="10" r="5.2" fill="#fff" />
        <circle cx="20.8" cy="10" r="4.2" fill="#e30a17" />
        <path d="m27 10 4.6-1.5-2.9 3.9V7.6l2.9 3.9z" fill="#fff" />
      </svg>
    );
  return (
    <svg {...box} aria-hidden>
...
/** bare folder — the files fan out and up on hover */
function Fan() {
  return (
    <div className="gg-fan">
      {FILES.map((f, i) => (
        <figure key={f.id} className="gg-file" style={{ ["--i" as string]: String(i - (FILES.length - 1) / 2) }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={f.image} alt="" draggable={false} />
          <figcaption>{f.title}</figcaption>
        </figure>
      ))}
      <span className="gg-fan-pocket" aria-hidden />
    </div>
  );
}
...
export default function PlatformGridSection() {
  ...
  return (
    <section className="gg-root">
      <style>{CSS}</style>

      <div className="gg-content">
        <header className="gg-head">
          <p className="gg-kicker">Your care concierge</p>
          <h2 className="gg-title">One AI. The whole journey.</h2>
        </header>

        <div ref={gridRef} className={"gg-grid" + (live ? " gg-in" : "")}>
          {/* ── portrait + conversation ── */}
          <div className="gg-card gg-girl" style={{ ["--d" as string]: "0ms" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="gg-thread">
              {THREAD.map((m, i) => (
                <p key={i} className={"gg-bubble gg-bubble--" + m.from + (i < shown ? " is-in" : "")}>
                  {m.text}
                </p>
              ))}
            </div>
            ...
            <div className="gg-girl-copy">
              ...
              <p>
                Doctors, hospitals and travel desks coordinated by one agent — so a treatment abroad
                takes a conversation, not a hundred emails.
              </p>
            </div>
          </div>

          {/* ── languages ── */}
          <div className="gg-card gg-lang" style={{ ["--d" as string]: "80ms" }}>
          ...
          {/* ── journeys planned ── */}
          <div className="gg-card gg-stat" style={{ ["--d" as string]: "160ms" }}>
            <div className="gg-folder">
              <Fan />
            </div>
            <strong>12K</strong>
            <span className="gg-stat-label">Journeys planned</span>
          </div>

          {/* ── multitasking ── */}
          <div className="gg-card gg-multi" style={{ ["--d" as string]: "240ms" }}>
            <div className="gg-rows">
              ...
              <div className="gg-lead-wrap">
                ...
                <div className="gg-lead-row">
                  ...
                  Match you with accredited hospitals worldwide
                </div>
              </div>
              {TASKS.map((t) => (
            ...
            <div className="gg-foot">
              <span>Everything a coordinator would do</span>
              <h3>Multitasking</h3>
            </div>
          </div>

          {/* ── omni channel ── */}
          <div className="gg-card gg-omni" style={{ ["--d" as string]: "320ms" }}>
            <div className="gg-foot gg-foot--top">
              <span>Chat, video consult, email</span>
              <h3>Omni channel</h3>
            </div>
...
.gg-root {
  ...
  background: #ececed;
  color: #14141a;
  padding: clamp(56px, 9vh, 110px) 0 clamp(64px, 10vh, 124px);
...
/* ── stat ── */
/* the fan needs somewhere to open into — up and out over the card's edge */
.gg-folder {
  position: relative;
  z-index: 2;
  display: grid;
...
/* ── the folder fan ── */
.gg-fan {
  --w: clamp(74px, min(8vw, 14vh), 116px);
  position: relative;
  width: var(--w);
  height: calc(var(--w) * 0.82);
  cursor: pointer;
}
.gg-file {
  position: absolute;
  ...
  box-shadow: 0 4px 14px -6px rgba(20, 20, 40, 0.45);
  transform: translateX(-50%) translateX(calc(var(--i) * 13%)) rotate(calc(var(--i) * 4.5deg));
  transform-origin: 50% 120%;
  transition: transform 0.46s cubic-bezier(0.34, 1.3, 0.64, 1), box-shadow 0.4s ease;
}
.gg-file img { width: 100%; height: 100%; object-fit: cover; display: block; }
.gg-file figcaption {
  position: absolute;
  inset: auto 0 0 0;
  padding: 8% 6%;
...
/* the pocket the files sit in */
.gg-fan-pocket {
  position: absolute;
  inset: auto 0 0 0;
  height: 56%;
  border-radius: 10% 10% 14% 14% / 14%;
  background: linear-gradient(165deg, #6b72dd, #4a51c4);
  box-shadow: 0 6px 18px -8px rgba(75, 82, 200, 0.85);
}
.gg-fan-pocket::before {
  content: "";
  position: absolute;
  left: 6%; top: -14%;
  width: 42%; height: 22%;
...
.gg-fan:hover .gg-file {
  transform: translateX(-50%) translate(calc(var(--i) * 80%), -40%) rotate(calc(var(--i) * 12deg));
  box-shadow: 0 12px 26px -10px rgba(20, 20, 40, 0.5);
}
.gg-fan:hover .gg-file figcaption { opacity: 1; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .gg-eq i { height: 60%; }
  .gg-bubble, .gg-sw, .gg-sw-knob, .gg-sw-track, .gg-sw-code, .gg-chip, .gg-file { transition: none; }
}
`;
